...
class TransformerRec(nn.Module):
    def __init__(self, num_users, num_items, embed_dim=64, n_heads=4):
        super().__init__()
    ...
    if os.path.isfile(MODEL):
        ...
        for epoch in range(epochs):
            ...
            for row in train_df.itertuples():
                user_id = torch.tensor([row.user_idx])
                item_seq = torch.tensor([row.item_idx]).unsqueeze(0)
                label = torch.tensor([row.clicked], dtype=torch.float)

                seq_user_emb = model(item_seq)          # [1, D]
                user_emb = model.user_emb(user_id)      # [1, D]
                final_user_emb = seq_user_emb + user_emb

                item_emb = model.item_emb(item_seq).squeeze(1)

                score = (final_user_emb * item_emb).sum(dim=1) # [1]
